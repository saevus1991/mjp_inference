...
filt = mjpi.KrylovFilterMem(master_equation, obs_model, t_obs, observations, initial_dist, rates, obs_param)
...
def fun(x):
    filt = mjpi.KrylovFilterMem(master_equation, obs_model, t_obs, observations, initial_dist, x, obs_param)
    llh = filt.log_prob()
    return(np.array([llh]))
